...
bool MainScene::init() {
    ...
    // 播放新场景的背景音乐
    CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("sounds/mainscene.mp3", true);
    return true;
}

void MainScene::menuReplaceCallback(Ref* pSender) {
    Director::getInstance()->replaceScene(TransitionFade::create(3, Tower_ex_Scene::createScene()));
}
