...
bool PlayScene::init() {
    ...
    initTowers_jinVector((map->getContentSize().height - visibleSize.height) / 2);
    // 创建粒子系统
    ...
    auto removeEnemy = CallFunc::create([particleSystem]() {
        particleSystem->removeFromParent(); // 从父节点中移除光线
        });
    ...
    auto towerIcon01 = Sprite::create("amiya01.png"); // 炮塔图标
    towerIcon01->setPosition(Vec2(896, 100)); // 设置初始位置
    this->addChild(towerIcon01);
    Vec2 originalPosition = towerIcon01->getPosition();
    auto touchListener01 = EventListenerTouchOneByOne::create();
    touchListener01->onTouchBegan = [towerIcon01](Touch* touch, Event* event) {
        if (towerIcon01->getBoundingBox().containsPoint(touch->getLocation())) {
            // 当触摸开始在炮塔图标上时
            return true; // 捕获触摸事件
        }
        return false;
    ...
    touchListener01->onTouchMoved = [this, towerIcon01](Touch* touch, Event* event) {
        // 更新炮塔图标的位置
        towerIcon01->setPosition(towerIcon01->getPosition() + touch->getDelta());
    };

    touchListener01->onTouchEnded = [this, towerIcon01](Touch* touch, Event* event) {
        bool isPlaced = false;
        for (auto& towerSpot : tower_position_jin) {
            if (towerSpot->getBoundingBox().containsPoint(towerIcon01->getPosition())) {
                
                towerIcon01->setPosition(towerSpot->getPosition());
        ...
        //towerIcon01->setPosition(Vec2(896, 100));
    };

    _eventDispatcher->addEventListenerWithSceneGraphPriority(touchListener01, towerIcon);
    auto mouseListener = EventListenerMouse::create();
    mouseListener->onMouseUp = [this, towerIcon01, originalPosition](EventMouse* event) {
        // 检查是否是鼠标右键
        if (event->getMouseButton() == EventMouse::MouseButton::BUTTON_RIGHT) {
            // 将炮塔图标放回初始位置
            towerIcon01->setPosition(originalPosition);
        }
    };
    ...
    touchListener->onTouchEnded = [this, towerIcon](Touch* touch, Event* event) {
        ...
        for (auto& towerSpot : tower_position_yuan) {
            if (towerSpot->getBoundingBox().containsPoint(towerIcon->getPosition())) {
                ...
                towerIcon->setPosition(towerSpot->getPosition());
...
void PlayScene::initTowers_jinVector(float off_position) {
